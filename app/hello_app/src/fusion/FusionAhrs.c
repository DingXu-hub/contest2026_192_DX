...
void FusionAhrsSetSettings(FusionAhrs *const ahrs, const FusionAhrsSettings *const settings) {
    ...
    ahrs->magneticRecoveryTimeout = ahrs->rejectionTimeout; /* DSH patch */
...
void FusionAhrsRestart(FusionAhrs *const ahrs) {
    ...
    ahrs->magneticRecoveryThreshold = ahrs->magneticRecoveryTimeout;
...
void FusionAhrsUpdate(FusionAhrs *const ahrs, const FusionVector gyroscope, const FusionVector accelerometer, const FusionVector magnetometer) {
    ...
    /* DSH patch: remember that a real magnetometer (not a forced zero)
     * has provided the heading; a later mag-drop during startup then
     * keeps the established yaw instead of snapping it back to zero. */
    if (FusionVectorIsZero(magnetometer) == false) {
        ahrs->headingAnchored = true;
    }
...
static inline FusionVector HalfHeadingFeedback(FusionAhrs *const ahrs, const FusionVector halfGravity, const FusionVector magnetometer) {
    ...
    if (FusionVectorIsZero(magnetometer) == false) {
        ...
        ahrs->magneticRecoveryTrigger = Clamp(ahrs->magneticRecoveryTrigger, 0, ahrs->magneticRecoveryTimeout);
...
void FusionAhrsUpdateNoMagnetometer(FusionAhrs *const ahrs, const FusionVector gyroscope, const FusionVector accelerometer) {
    ...
    // Zero heading during startup so a gyro-only boot has a defined yaw.
    // DSH patch: only when no magnetometer has ever anchored the heading.
    // The watch feeds the mag for its first 2.5 s (still inside the 3 s
    // startup window) and then gates it out when the field is weak; that
    // drop must keep the established heading, not zero it.
    if (ahrs->startup && (ahrs->headingAnchored == false)) {
