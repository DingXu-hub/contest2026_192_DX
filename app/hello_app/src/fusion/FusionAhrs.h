...
typedef struct {
    ...
    bool headingAnchored;   /* DSH patch: true once a non-zero magnetometer
                             * has established the yaw; UpdateNoMagnetometer
                             * must not zero an anchored heading */
    ...
    int32_t magneticRecoveryTimeout;   /* sample count; DSH patch to make
                                        * mag recovery slower than accel */
