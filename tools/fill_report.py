...
abstract = (
    ...
    "运行时 Skill（/data/agent/skills/ 动态加载与关键词触发，实测“天气”经网关取回真实天气）以及经 PC 网关的可靠联网"
    "（真实时间同步、HTTP 取数、LLM 问答）。姿态解算采用**自验证的 6 轴方案**（陀螺+加速度）：滚转/俯仰由重力给出绝对基准，"
    "航向为重力投影积分的相对方位；模块内置合成数据自检（9 个用例），设备上 `!att test` 逐个打印 PASS/FAIL，"
    "初版被该测试抓出滚转与偏航积分两处符号错误。针对 CH340 链路“控制台 RX 等效 1 字节深”与 NSH 抢占控制台两个隐蔽根因，"
    "自研帧协议（CRC16+ACK/重传）+ PC 侧逐字节节流写实现命令零丢失。"
    "资源上关闭 ALLSYMS 与缩减缓冲，SRAM 占用由 81.8% 降到 38.2%。"
)
fill(P[7], [abstract])
...
fill(P[12], [
    ...
    "② 姿态解算的自证式验收：把“符号/单位/坐标系是否正确”交给设备内的合成数据自检（`!att test`，9 个用例）判定，"
    "而不是靠推理；并以重力为绝对基准给出滚转/俯仰，航向明确标注为相对方位（6 轴固有）；",
...
fill(P[14], [
    ...
    "③ 应用层：渲染与 UI（自研工具层 + 帧缓冲直写 + ePicasso GPU 纯色填充）、传感器与 6 轴姿态（官方 Fusion 链路，见 3.3.7）、"
    "跑步引擎（步数/配速/航位推算轨迹）、AI Agent（工具执行 + 主动引擎）；"
    "④ 链路与云侧：`link.c` 帧协议 ⇄ PC 网关（时间同步/HTTP 代理/LLM/文件通道预留）。",
    "数据流：IMU → 采样任务 → 6 轴姿态（官方 Fusion 链路）→ 姿态/相对航向 → UI 与跑步引擎（航位推算轨迹）；"
    "磁力计不再参与航向，仅用于诊断与校准展示；"
...
fill(P[19], [
    "① 姿态解算（最终方案）：**仅用 6 轴（陀螺 + 加速度），且数学部分直接用官方库**。滚转/俯仰来自"
    "xioTechnologies/Fusion 的四元数半重力角度残差反馈，航向为**相对方位**（无磁力计），由同一四元数积分得到；"
    "`src/attitude6.c` 只是官方链路的薄封装：`FusionBiasUpdate`（静止门控零偏低通，3 °/s 门限 + 3 s 静止期）"
    "→ `FusionAhrsUpdateNoMagnetometer`（`FusionConventionNed`、`gyroscopeRange=2000 dps`、"
    "`accelerationRejection=15°`、`rejectionTimeout=5 s`，每帧按真实 dt 调 `FusionAhrsSetSamplePeriod()`）"
    "→ `FusionQuaternionToEuler`。手腕绕前臂的摆动由重力投影自然消掉。",
    "② 为什么不用磁力计：本机磁力计硬铁误差最大达 27 µT、工作台环境磁场达地磁的 3.7 倍，"
    "磁航向无法在本环境获得可信验证（详见 docs/指南针病因分析）；与其让航向被错误磁场拽走，"
    "产品路径选择可被物理验证的 6 轴相对航向，并在 UI 明确标注“6-AXIS REL”与提供 SET 0 归零。",
    "③ 自证式验证（关键）：`src/attitude6.c` 内置**合成数据自检**——用已知姿态"
    "（a=(sinθ, −cosθsinφ, −cosθcosφ) g）与已知竖直轴角速度生成输入，检查**官方链路**能否还原真值，"
    "覆盖 平放/右倾 30°/左倾 45°/抬头 40°/滚转+俯仰/±60 °/s 偏航积分/带倾角转弯/毛刺抑制 共 9 个用例，"
    "设备上执行 `!att test` 逐个打印 [PASS]/[FAIL]（实测 9/9 PASS）。该测试固定了三个事实："
    "① 官方 NED 的 roll/pitch 约定与本机实测完全一致；② 官方无磁路径在启动 3 s 内会把航向强制归零"
    "（自检用例必须先预热）；③ 毛刺抑制不会吞掉真实转动。"
    "**先怀疑自己的代码**：正因这个测试，发现并修掉了自写毛刺过滤的缺陷——EMA 停在 0 时"
    "“从静止开始的一次真实转身”会被整段丢弃，现改为连续 3 个超阈样本即判为真实运动（迟滞，150 ms 延迟）。",
    "④ 漂移与毛刺抑制：官方库假定陀螺信号干净（其超量程检查只在 ±1960 dps 附近触发），抓不到本板实测的"
    "18–30 dps 单样本读毛刺，因此送入官方链路前做幅值迟滞过滤（>4×EMA+3 dps 丢弃，连续 3 个超阈即判为真实运动）；"
    "静止段另以静止检测把残余角速度强制为零。真机 3 分钟静止实测：yaw 漂移 +0.10°~+0.30°（+0.03~+0.10 °/min），"
    "期间拦截 1 次输入毛刺；对照加毛刺过滤前为 +1.2 °/min（阶梯状）。",
    "⑤ 主动决策：定时器到期或空闲超时触发；触发后按“先本地工具、再云端问答”的顺序执行，"
    "并把行为写入 @AI/@TOOL 日志便于复核。",
])
fill(P[20], [
...
fill(P[23], [
    ...
    "② 融合流：每帧 dt → `FusionBiasUpdate`（零偏）→ `FusionAhrsUpdateNoMagnetometer`（四元数 + 半重力反馈 +"
    "角度门限的加速度拒绝）→ `FusionQuaternionToEuler` → UI 航向；送入前先过自写毛刺迟滞过滤（见 3.3.7）；",
...
fill(P[26], [
    "自建开发 Skill：`skills/huangshan-watch-dev/SKILL.md`（本仓，约 100 行）——沉淀“本地改→VM 编译→烧录→串口/截图验证”全流程、"
    "硬件拓扑与全部踩坑（含控制台 RX 1 字节深、NSH 抢占 console、PM 门控 codec 时钟、构建目标更名等），供后续任何 AI 会话直接复用。",
    "运行时 Skill（AI 硬件赛道要求，**已实现**）：`app/hello_app/src/runtime_skill.c` 在启动时于 `/data/agent/skills/` 播种并扫描 markdown 技能，"
    "解析 `name:` / `trigger:` / `action:` 三个字段后注册为两类入口——① 命令式 `!skills`（列出）与 `!skill <name>`（执行）；"
    "② 关键词式：用户在串口/网关输入的自由文本命中 `trigger` 即自动执行（例如说“天气”自动取回天气、说“喝水/补水”弹出补水提醒、说“拉伸”启动 5 分钟提醒）。",
    "技能动作被限制在 Agent 已有能力内（`notify` 通知卡片 / `timer` 提醒 / `status` 状态播报 / `tip` LLM 提示 / `http` 经网关取数），"
    "因此技能文件不会执行任意代码；目录内新增 .md 后可用 `!skill reload` 热加载，无需重启。"
    "仓内示例见 `skills/runtime/`（hydrate / stretch / weather），板子 `/data` 为 tmpfs，故每次启动自动播种。",
])
# ---------------- 3.5 系统测试与结果分析 ----------------
fill(P[28], [
...
fill(P[30], [
    "性能实测数据（均附测量方式与日志）：",
    "· 资源：Flash 5,805,180 B / 16 MB = 34.6%；SRAM 200,464 B / 512 KB = 38.2%（优化前 428,800 B = 81.8%）；"
    ...
    "· 姿态（6 轴，官方 Fusion 链路）：`!att test` 合成自检 **9/9 用例 PASS**；真机 3 分钟静止实测 yaw 漂移"
    "+0.10°~+0.30°（+0.03~+0.10 °/min），期间拦截 1 次输入毛刺（日志 `tools/att_still.py` 输出）；"
    "物理核对（tools/attitude_test.py）：屏幕朝上平放 roll +0.5~+1.5°/pitch≈0；屏幕朝下扣放 |roll| ≈ 179.8°"
    "（证实坐标系/符号模型）；滤波值与独立加速度参考平均偏差 0.33°；航向进入跑步航位推算（方位角死区 4°→1°）。",
])
fill(P[31], [
    "可靠性/稳定性：",
...
# ---------------- 3.7 总结与展望 ----------------
fill(P[35], [
    "① 完成一套可运行、可复现的手表固件：6 页现代化 UI、6 轴姿态（官方 Fusion 链路 + 合成自检）与磁自校准、端侧主动 AI Agent、"
    "经 PC 网关的可靠联网（时间同步/HTTP/LLM），并在真机上逐项验证；",
    "② 解决了两个隐蔽的链路根因（控制台 RX 1 字节深、NSH 抢占 console），命令零丢失；"
    "③ 磁力计校准以物理不变量做验收，Fusion 磁误差 90°→0–1°；④ 资源占用 SRAM 81.8%→38.2%；"
...
fill(P[37], [
    ...
    "④ 电池监测未实现（AW32001 在 I2C 上，驱动未写）；",
    "⑤ 6 轴方案的固有局限：航向为相对方位（无绝对北向），需用户 SET 0 归零，且长时间无运动时仍有缓慢漂移；"
    "若后续能在洁净磁场环境下重新标定磁力计（或换用带软磁屏蔽的结构），可再引入绝对航向；"
    "⑥ 端侧模型：当前主动判断为规则/状态机实现，未来可在支持 NPU 的芯片上落地量化小模型；"
    "运行时 Skill 现已支持动态加载，后续可扩展更多动作类型（如本地传感器编排、UI 场景切换）。",
])
